...
- **Only `lib/edupage/http.ts`, `lib/analytics/http.ts`, `lib/share/native.ts` and
  `lib/widget/native.ts` may import `@capacitor/*`.** ESLint `no-restricted-imports` enforces
  this per-module; everything else stays
...
- Relative imports carry the `.ts`/`.tsx` extension; `@/…` aliases don't.
